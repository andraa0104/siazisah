    ...
    <div class="flex-1 flex flex-col overflow-hidden">
      ...
      <main class="flex-1 overflow-y-auto p-4 md:p-6">
        ...
        <div class="mt-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3 text-sm text-gray-600">
          <div>
            Menampilkan {{ masjidList.length }} dari {{ pagination.total }} data
          </div>
...
const loadMasjid = async () => {
  ...
  try {
    ...
    const { data } = await api.getMasjid(params);
    if (data.success) {
      const payload = data.data || {};
      if (Array.isArray(payload.items)) {
        masjidList.value = payload.items;
        syncPagination(payload, payload.items.length);
      } else if (Array.isArray(payload)) {
        masjidList.value = applyLocalPagination(payload);
      } else {
        masjidList.value = [];
