  ...
  <div class="flex h-screen overflow-hidden">
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
      ...
      <main class="flex-1 overflow-y-auto p-4 md:p-6">
          ...
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              ...
              >
                ...
                <td class="px-6 py-4">
                  <span
                    :class="`px-2 py-1 text-xs font-semibold rounded-full ${getZakatBadge(t.jenis_zakat)}`"
                  >
        ...
        <div class="md:hidden space-y-3">
          ...
          >
            ...
            <div class="flex gap-2">
              ...
              <button
                @click="printReceipt(t.id)"
                class="flex-1 bg-green-50 text-green-600 py-2 rounded-lg hover:bg-green-100 text-sm"
              >
        ...
        >
          <div>
            Menampilkan {{ transaksiList.length }} dari
    ...
    <Modal :show="showModal" title="Input Transaksi Zakat" @close="closeModal">
      <form @submit.prevent="saveTransaksi" class="space-y-4">
        ...
        <div
          v-if="form.jenis_zakat === 'fitrah' || form.jenis_zakat === 'fidyah'"
        >
          <label class="block text-sm font-medium text-gray-700 mb-2"
            >Bentuk
            {{ form.jenis_zakat === "fidyah" ? "Fidyah" : "Zakat" }} *</label
          >
          <select
            v-model="form.bentuk_zakat"
            required
        ...
        <div v-if="form.jenis_zakat === 'mal'">
          ...
          >
          <select
            v-model="form.jenis_harta"
            required
            @change="setPersentase"
            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500"
          >
            <option value="">Pilih Jenis Harta</option>
            <option
              v-for="item in malRateOptions"
              :key="item.key"
              :value="item.key"
            >
          ...
          <p
            v-if="malRateOptions.length === 0"
            class="text-xs text-red-500 mt-1"
          >
            Jenis zakat mal belum diatur di halaman Pengaturan.
          </p>
        </div>
        <div v-if="form.jenis_harta">
          ...
          >
          <input
            v-model.number="form.nominal_harta"
            type="number"
            required
            min="0"
            @input="calculateMal"
          ...
          <p class="text-xs text-gray-500 mt-1">
            Persentase zakat: {{ formatPercent(form.persentase_zakat) }}%
          </p>
        </div>
        <div
          v-if="form.bentuk_zakat === 'uang' && form.jenis_zakat === 'fitrah'"
        >
          <label class="block text-sm font-medium text-gray-700 mb-2"
            >Kelas Zakat *</label
          >
          <select
            v-model="form.kelas_zakat"
            required
            @change="calculateTotal"
            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500"
          >
            ...
            <option value="1">
              Kelas 1 - Rp {{ formatNumber(kadarZakat.kelas1) }}
            </option>
            <option value="2">
              Kelas 2 - Rp {{ formatNumber(kadarZakat.kelas2) }}
            </option>
            <option value="3">
              Kelas 3 - Rp {{ formatNumber(kadarZakat.kelas3) }}
            </option>
          </select>
        </div>
        <div
          v-if="form.bentuk_zakat === 'uang' && form.jenis_zakat === 'fidyah'"
        >
          <label class="block text-sm font-medium text-gray-700 mb-2"
            >Jumlah Hari yang Ditinggalkan *</label
          >
          <input
            v-model.number="form.jumlah_hari_fidyah"
            type="number"
            required
            min="1"
            @input="calculateFidyahTotal"
            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500"
            placeholder="Masukkan jumlah hari"
          />
          <p class="text-xs text-gray-500 mt-1">
            Fidyah per hari setara dengan 3/4 gantang beras ({{
              formatNumber(kadarZakat.fidyahPerHari)
            }}/hari)
          </p>
        </div>
        <div
          v-if="form.bentuk_zakat === 'beras' && form.jenis_zakat === 'fitrah'"
        >
          <label class="block text-sm font-medium text-gray-700 mb-2"
            >Standar Beras per Jiwa (kg) *</label
          >
          <input
            v-model.number="form.standar_beras_per_jiwa"
            type="number"
            step="0.1"
            min="0.1"
            required
            @input="calculateBerasTotal"
            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500"
            placeholder="Contoh: 2.5 atau 3"
          />
          <p class="text-xs text-gray-500 mt-1">
            Default dari pengaturan: {{ kadarZakat.fitrahBerasPerJiwa }} kg per
            jiwa
          </p>
        </div>
        <div
          v-if="form.bentuk_zakat === 'beras' && form.jenis_zakat === 'fidyah'"
        >
          <label class="block text-sm font-medium text-gray-700 mb-2"
            >Jumlah Hari Fidyah *</label
          >
          <input
            v-model.number="form.jumlah_hari_fidyah"
            type="number"
            required
            min="1"
            @input="calculateFidyahBerasTotal"
            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500"
            placeholder="Masukkan jumlah hari"
          />
          <p class="text-xs text-gray-500 mt-1">
            Fidyah beras per hari: {{ kadarZakat.fidyahBerasPerHari }} kg
          </p>
        </div>
        <div v-if="form.jenis_zakat === 'fitrah'">
          ...
          >
          <input
            v-model.number="form.jumlah_orang"
            type="number"
            required
            min="1"
        ...
        <div
          v-if="
            form.jenis_zakat === 'fidyah' &&
            totalWajib > 0 &&
            form.bentuk_zakat === 'uang'
          "
        >
          <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
            ...
            <p class="text-xs text-yellow-600 mt-1">
              {{ form.jumlah_hari_fidyah }} hari × Rp
              {{ formatNumber(kadarZakat.fidyahPerHari) }}
            </p>
          </div>
        </div>
        ...
        >
          <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
            ...
            <p class="text-xs text-yellow-600 mt-1">
              {{ form.jumlah_hari_fidyah }} hari ×
              {{ kadarZakat.fidyahBerasPerHari }} kg
            </p>
          </div>
        </div>
        <div
          v-if="
            form.jenis_zakat === 'fitrah' &&
            form.bentuk_zakat === 'beras' &&
            form.standar_beras_per_jiwa &&
            form.jumlah_orang
          "
        >
          <div class="bg-green-50 border border-green-200 rounded-lg p-4">
            ...
            <p class="text-xs text-green-600 mt-1">
              {{ form.jumlah_orang }} orang ×
              {{ form.standar_beras_per_jiwa }} kg
            </p>
          </div>
        </div>
        <div v-if="form.bentuk_zakat === 'beras'">
          <label class="block text-sm font-medium text-gray-700 mb-2"
            >Kilogram Beras yang Dibayarkan *</label
          >
          <input
            v-model.number="form.kg_beras_dibayar"
            type="number"
            step="0.1"
            min="0.1"
            required
            @input="calculateBerasInfaq"
            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500"
            placeholder="Masukkan jumlah kg beras"
          />
        </div>
        <div v-if="form.bentuk_zakat === 'beras' && form.kg_beras_dibayar">
          <label class="block text-sm font-medium text-gray-700 mb-2"
            >Harga Beras per Kg (Opsional)</label
          >
          <input
            v-model.number="form.harga_beras_per_kg"
            type="number"
            min="0"
            @input="calculateBerasRupiah"
            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500"
            placeholder="Untuk konversi ke Rupiah"
          />
          <p class="text-xs text-gray-500 mt-1">
            Isi jika ingin mencatat nilai dalam rupiah
          </p>
        </div>
        <div
          v-if="totalWajib > 0 && form.bentuk_zakat !== 'beras'"
          class="bg-gray-100 p-4 rounded-lg"
        >
          <label class="block text-sm font-medium text-gray-700 mb-2">{{
            form.jenis_zakat === "mal" ? "Total Zakat Mal" : "Total Wajib Zakat"
          }}</label>
          <div class="text-2xl font-bold text-green-600">
            {{ formatCurrency(totalWajib) }}
          </div>
          <p
            v-if="form.jenis_zakat === 'mal'"
            class="text-xs text-gray-600 mt-1"
          >
            {{ formatCurrency(form.nominal_harta) }} ×
            {{ form.persentase_zakat }}%
          </p>
        </div>
        <div v-if="form.bentuk_zakat !== 'beras'">
          <label class="block text-sm font-medium text-gray-700 mb-2"
          ...
          >
          <input
            v-model.number="form.total_dibayar"
            type="number"
            required
            min="0"
        ...
        <div
          v-if="
            form.bentuk_zakat === 'beras' &&
            form.harga_beras_per_kg &&
            form.kg_beras_dibayar
          "
        >
          <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
            ...
            <p class="text-2xl font-bold text-blue-700">
              {{ formatCurrency(totalBerasRupiah) }}
            </p>
            <p class="text-xs text-blue-600 mt-1">
              {{ form.kg_beras_dibayar }} kg × Rp
              {{ formatNumber(form.harga_beras_per_kg) }}
            </p>
          </div>
        </div>
        <div
          v-if="infaqTambahan > 0 && form.bentuk_zakat !== 'beras'"
        ...
        <div class="flex gap-3 pt-4">
          <button
            type="submit"
            :disabled="isSaving"
            class="flex-1 bg-green-600 text-white py-3 rounded-lg hover:bg-green-700 transition font-medium disabled:opacity-50"
          >
    ...
    <Modal
      :show="showPrintDataModal"
      title="Print Laporan Data"
      @close="showPrintDataModal = false"
    >
      <form @submit.prevent="printRekapData" class="space-y-4">
        ...
        <div class="flex gap-3 pt-2">
          ...
          <button
            type="button"
            @click="showPrintDataModal = false"
    ...
    >
      <div v-if="selectedTransaction" class="space-y-4">
          ...
          <div class="space-y-2 text-sm">
            <div class="flex justify-between">
              ...
              <span class="font-medium text-gray-900">{{
                selectedTransaction.muzakki_nama
              }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-gray-600">Alamat:</span>
              <span class="font-medium text-gray-900">{{
                selectedTransaction.muzakki_alamat
              }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-gray-600">Telepon:</span>
              <span class="font-medium text-gray-900">{{
                selectedTransaction.muzakki_telepon || "-"
              }}</span>
            </div>
          </div>
        </div>
        ...
        <div class="bg-green-50 rounded-lg p-4">
          ...
          <div class="space-y-2 text-sm">
            <div class="flex justify-between">
              ...
              <span class="font-medium text-gray-900">{{
                formatDate(selectedTransaction.tanggal_bayar)
              }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-gray-600">Jenis Zakat:</span>
              <span
                :class="`px-2 py-1 text-xs font-semibold rounded-full ${getZakatBadge(selectedTransaction.jenis_zakat)}`"
              >
            ...
            <div class="flex justify-between">
              ...
              <span class="font-medium text-gray-900">{{
                selectedTransaction.bentuk_zakat || "-"
              }}</span>
            </div>
          </div>
        </div>
        ...
        <!-- Detail Zakat Fitrah -->
        <div
          v-if="selectedTransaction.jenis_zakat === 'fitrah'"
          class="bg-blue-50 rounded-lg p-4"
        >
          <h3 class="font-semibold text-gray-800 mb-3 flex items-center">
            <i class="fas fa-info-circle mr-2 text-blue-600"></i>Detail Zakat
            Fitrah
          </h3>
          ...
          <div class="bg-white rounded-lg p-3 mb-3 border-2 border-blue-300">
            <div class="flex justify-between items-center">
              <span class="text-gray-700 font-semibold"
                >Jumlah Orang yang Dizakati:</span
              >
          ...
          <div class="space-y-2 text-sm">
            ...
            >
              ...
              <span class="font-medium text-gray-900"
                >Kelas {{ selectedTransaction.kelas_zakat }}</span
              >
            </div>
            <div
            ...
            >
              ...
              <span class="font-medium text-gray-900"
                >{{
                  selectedTransaction.standar_beras_per_jiwa ||
                  kadarZakat.fitrahBerasPerJiwa ||
                  2.5
                }}
                kg</span
              >
              ...
              <span class="font-medium text-blue-700"
                >{{
                  (
                    (selectedTransaction.standar_beras_per_jiwa ||
                      kadarZakat.fitrahBerasPerJiwa ||
                      2.5) * selectedTransaction.jumlah_orang
                  ).toFixed(1)
                }}
                kg</span
              >
            ...
            >
              ...
              <span class="font-medium text-gray-900"
                >{{
                  selectedTransaction.kg_beras_dibayar ||
                  extractKgBerasFromKeterangan(selectedTransaction.keterangan)
                }}
                kg</span
              >
            </div>
            ...
            >
              ...
              <span class="font-medium text-gray-900">{{
                formatCurrency(selectedTransaction.harga_beras_per_kg || 0)
              }}</span>
            </div>
          </div>
        </div>
        ...
        <!-- Detail Zakat Mal -->
        <div
          v-if="selectedTransaction.jenis_zakat === 'mal'"
          class="bg-blue-50 rounded-lg p-4"
        >
          <h3 class="font-semibold text-gray-800 mb-3 flex items-center">
            <i class="fas fa-info-circle mr-2 text-blue-600"></i>Detail Zakat
          ...
          <div class="space-y-3">
            ...
            <div class="bg-white rounded-lg p-3">
              <div class="flex justify-between items-center">
                ...
                <span class="font-semibold text-gray-900 text-lg">{{
                  getMalJenisLabel(selectedTransaction.jenis_harta)
                }}</span>
              </div>
            </div>
            ...
            <div class="bg-white rounded-lg p-3 space-y-2">
              <div class="flex justify-between items-center pb-2">
                ...
                <span class="font-medium text-gray-900">{{
                  formatCurrency(selectedTransaction.nominal_harta)
                }}</span>
              </div>
              <div class="flex justify-between items-center pb-2">
                <span class="text-sm text-gray-600">Persentase Zakat:</span>
                <span class="font-medium text-gray-900"
                  >{{ selectedTransaction.persentase_zakat }}%</span
                >
              </div>
              <div class="border-t border-gray-200 pt-2">
                <div class="flex justify-between items-center">
                  <span class="text-sm font-semibold text-blue-700"
                    >Zakat yang Wajib:</span
                  >
                  <span class="font-bold text-blue-700">{{
                    formatCurrency(
                      (selectedTransaction.nominal_harta *
                        selectedTransaction.persentase_zakat) /
                        100,
                    )
                  }}</span>
                </div>
            ...
            <div class="bg-blue-100 rounded-lg p-3">
              <div class="flex items-start">
                ...
                <div class="text-xs text-blue-800">
                  ...
                  <p v-else>Zakat dikeluarkan sesuai ketentuan syariat Islam</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <!-- Detail Fidyah -->
        <div
          v-if="selectedTransaction.jenis_zakat === 'fidyah'"
        ...
        >
          ...
          <div class="bg-white rounded-lg p-3 mb-3 border-2 border-yellow-300">
            <div class="flex justify-between items-center">
              <span class="text-gray-700 font-semibold"
                >Jumlah Hari Fidyah:</span
              >
              <span class="text-2xl font-bold text-yellow-700"
                >{{ selectedTransaction.jumlah_hari_fidyah || 0 }} Hari</span
              >
            </div>
          </div>
          ...
          <div class="space-y-2 text-sm">
            <div class="flex justify-between">
              ...
              <span class="font-medium text-gray-900">{{
                selectedTransaction.bentuk_zakat === "uang" ? "Uang" : "Beras"
              }}</span>
            </div>
            <div
              v-if="selectedTransaction.bentuk_zakat === 'uang'"
            ...
            >
              ...
              <span class="font-medium text-gray-900">{{
                formatCurrency(kadarZakat.fidyahPerHari)
              }}</span>
            </div>
            <div
              v-if="selectedTransaction.bentuk_zakat === 'beras'"
            ...
            >
              ...
              <span class="font-medium text-gray-900"
                >{{ kadarZakat.fidyahBerasPerHari }} kg</span
              >
            </div>
            <div
              v-if="selectedTransaction.bentuk_zakat === 'beras'"
          ...
          <div class="bg-yellow-100 rounded-lg p-3 mt-3">
            <div class="flex items-start">
              ...
              <div class="text-xs text-yellow-800">
                ...
                <p>
                  Fidyah adalah denda atau ganti rugi bagi yang tidak dapat
                  berpuasa karena uzur (sakit permanen, tua, atau kondisi
                  lainnya).
                </p>
              </div>
            </div>
        ...
        <!-- Keterangan -->
        <div
          v-if="selectedTransaction.keterangan"
...
const malRateOptions = computed(() =>
  Object.entries(kadarZakat.value.malRates || {})
    .map(([key, value]) => ({
      key,
...
const totalWajib = computed(() => {
  if (
    form.value.jenis_zakat === "fitrah" &&
    form.value.bentuk_zakat === "uang" &&
    form.value.kelas_zakat &&
    form.value.jumlah_orang
  ) {
    const nominal = kadarZakat.value[`kelas${form.value.kelas_zakat}`];
    return nominal * form.value.jumlah_orang;
  }
  if (
    form.value.jenis_zakat === "mal" &&
    form.value.nominal_harta &&
    form.value.persentase_zakat
  ) {
...
const totalBerasWajib = computed(() => {
  if (
    form.value.jenis_zakat === "fitrah" &&
    form.value.bentuk_zakat === "beras" &&
    form.value.standar_beras_per_jiwa &&
    form.value.jumlah_orang
  ) {
...
const showBentukFilter = computed(
  () =>
    filters.value.jenis_zakat === "fitrah" ||
    filters.value.jenis_zakat === "fidyah",
);
...
const filterLocalTransaksi = (items) => {
  ...
  return items.filter((item) => {
    if (
      filters.value.jenis_zakat &&
      item.jenis_zakat !== filters.value.jenis_zakat
    ) {
      return false;
    }
...
const getDetail = (t) => {
  ...
  if (t.jenis_zakat === "mal")
    return `${getMalJenisLabel(t.jenis_harta)} - ${formatCurrency(t.nominal_harta)} (${t.persentase_zakat}%)`;
...
const setPersentase = () => {
  form.value.persentase_zakat = Number(
    kadarZakat.value.malRates?.[form.value.jenis_harta] || 0,
  );
  calculateMal();
};
...
const calculateMal = () => {
  if (form.value.nominal_harta && form.value.persentase_zakat) {
    const total =
      form.value.nominal_harta * (form.value.persentase_zakat / 100);
...
const calculateFidyahTotal = () => {
  if (form.value.jumlah_hari_fidyah) {
    form.value.total_dibayar =
      kadarZakat.value.fidyahPerHari * form.value.jumlah_hari_fidyah;
  }
};
...
const calculateFidyahBerasTotal = () => {
  if (form.value.jumlah_hari_fidyah) {
    ...
    form.value.kg_beras_dibayar =
      form.value.jumlah_hari_fidyah * kadarZakat.value.fidyahBerasPerHari;
  }
};
...
const calculateBerasInfaq = () => {
  ...
  if (form.value.kg_beras_dibayar && form.value.harga_beras_per_kg) {
    form.value.total_dibayar =
      form.value.kg_beras_dibayar * form.value.harga_beras_per_kg;
  }
};
...
const printReceipt = async (id) => {
  try {
    ...
    const response = await fetch(api.printTransaksi(id), {
...
const printRekapData = async () => {
  try {
    ...
    toast.value = {
      message: `Gagal membuka print data${status ? ` (${status})` : ""}: ${detail}`,
      type: "error",
    };
  }
};
...
const saveTransaksi = async () => {
  isSaving.value = true;
  try {
    const muzakkiData = await api.createMuzakki({
      nama: form.value.nama_muzakki,
      alamat: "-",
      telepon: "-",
    });
    ...
    if (form.value.jenis_zakat === "fitrah") {
      ...
      if (form.value.bentuk_zakat === "uang") {
        ...
        payload.nominal_per_orang =
          kadarZakat.value[`kelas${form.value.kelas_zakat}`];
